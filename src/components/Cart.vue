  ...
  <div class="container py-1 py-md-5">
    <div v-if="cart.cart.length>=1 ">
      ...
      <div class="row pb-5" v-for="product in cart.cart" :key="product.details.id" :product="product">
        ...
        <div class="col-2 align-self-center d-none d-md-block"><img :src="getImgUrl(product.details.id)" v-bind:alt="product.name" /></div>
        <div class="col-3 align-self-center d-none d-md-block">
          <router-link class="card-text" :to="{ name: 'productDetail' , params: {id: product.details.id}}">
            {{ product.details.producer }} {{ product.details.name }}
          </router-link>
        </div>
        <div class="col-2 align-self-center d-none d-md-block">{{ product.details.price.toLocaleString().replace(',', ' ') }} zł</div>
        ...
        <div class="col-2 align-self-center d-none d-md-block">{{ (product.details.price * product.quantity).toLocaleString().replace(',', ' ') }} zł</div>
